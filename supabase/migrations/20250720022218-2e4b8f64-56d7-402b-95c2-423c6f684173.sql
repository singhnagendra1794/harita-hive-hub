...
DROP FUNCTION IF EXISTS public.start_stream_session(uuid);
DROP FUNCTION IF EXISTS public.start_stream_session(uuid, text, text);
DROP FUNCTION IF EXISTS public.stop_stream_session(uuid);
DROP FUNCTION IF EXISTS public.update_stream_status(uuid, text, integer);
...
-- Create the generate_stream_key function
CREATE FUNCTION public.generate_stream_key(p_user_id UUID)
RETURNS TEXT AS $$
DECLARE
  new_key TEXT;
BEGIN
...
CREATE FUNCTION public.start_stream_session(
  p_user_id UUID,
...
CREATE FUNCTION public.update_stream_status(
  p_session_id UUID,
  p_status TEXT,
