-- Grant professional access and course enrollment for syedarshiya2110@gmail.com (case insensitive)

DO $$
DECLARE
  target_user_id UUID;
  target_course_id UUID := 'e7d5f6a1-2b3c-4d5e-6f7a-8b9c0d1e2f3a'; -- Geospatial Technology Unlocked course ID
BEGIN
  -- Find user by email (case insensitive)
  SELECT id INTO target_user_id 
  FROM auth.users 
  WHERE LOWER(email) = LOWER('syedarshiya2110@gmail.com');
  ...
  IF target_user_id IS NOT NULL THEN
    ...
    VALUES (target_user_id, target_course_id, now(), 0)
  ...
  ELSE
    RAISE NOTICE 'User with email syedarshiya2110@gmail.com not found';
  END IF;
END $$;