-- Create GEOVA teaching schedule and session tracking tables
CREATE TABLE public.geova_teaching_schedule (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  course_title TEXT NOT NULL DEFAULT 'Geospatial Technology Unlocked',
  ...
  scheduled_time TIME DEFAULT '05:00:00',
  duration_minutes INTEGER DEFAULT 120,
  status TEXT DEFAULT 'scheduled' CHECK (status IN ('scheduled', 'live', 'completed', 'cancelled')),
  session_id UUID REFERENCES public.live_classes(id),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);

-- Create GEOVA session interactions table to track Q&A and chat
CREATE TABLE public.geova_session_interactions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  schedule_id UUID REFERENCES public.geova_teaching_schedule(id),
  session_id UUID REFERENCES public.live_classes(id),
  user_id UUID REFERENCES auth.users(id),
  interaction_type TEXT NOT NULL CHECK (interaction_type IN ('question', 'answer', 'chat', 'feedback')),
...
-- Create GEOVA course progress tracking
CREATE TABLE public.geova_student_progress (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id),
  course_title TEXT NOT NULL DEFAULT 'Geospatial Technology Unlocked',
...
-- Enable RLS
ALTER TABLE public.geova_teaching_schedule ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.geova_session_interactions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.geova_student_progress ENABLE ROW LEVEL SECURITY;

-- Create policies for geova_teaching_schedule (public read, admin write)
CREATE POLICY "Anyone can view teaching schedule" 
ON public.geova_teaching_schedule 
FOR SELECT 
USING (true);

CREATE POLICY "Admin can manage teaching schedule" 
ON public.geova_teaching_schedule 
FOR ALL 
USING (public.is_super_admin());

-- Create policies for geova_session_interactions
CREATE POLICY "Users can view session interactions" 
ON public.geova_session_interactions 
FOR SELECT 
USING (true);
...
CREATE POLICY "Admin can manage all interactions" 
ON public.geova_session_interactions 
FOR ALL 
USING (public.is_super_admin());

-- Create policies for geova_student_progress
CREATE POLICY "Users can view their own progress" 
ON public.geova_student_progress 
FOR SELECT 
...
-- Create function to auto-generate daily GEOVA sessions
...
SELECT public.create_geova_daily_sessions();