-- Comprehensive update to ensure all professional users have correct access
DO $$
DECLARE
...
BEGIN
  ...
  FOR user_record IN 
    ...
    WHERE LOWER(au.email) = ANY(
      SELECT LOWER(unnest(pro_emails))
    )
  LOOP
  ...
  -- Verify and log any emails that don't have corresponding users
  FOR user_record IN 
    SELECT unnest(pro_emails) as email
    WHERE NOT EXISTS (
      SELECT 1 FROM auth.users au 
      WHERE LOWER(au.email) = LOWER(unnest(pro_emails))
    )
  LOOP
    RAISE NOTICE 'Email not found in users table: %', user_record.email;
  END LOOP;
END $$;