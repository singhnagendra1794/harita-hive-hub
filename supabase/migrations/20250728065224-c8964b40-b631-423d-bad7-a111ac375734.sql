...
-- Insert role notification email templates into existing email_templates table
...
BEGIN
  ...
  INTO target_user_name, user_subscription
  FROM auth.users au
  ...
  user_current_role := COALESCE(user_current_role, 'user');
  ...
  VALUES (target_email, user_current_role, new_role_name, auth.uid(), admin_email, change_reason);
  ...
  VALUES (
    target_user_id,
    CASE 
      WHEN user_current_role = 'user' AND new_role_name = 'professional' THEN 'welcome_professional'
