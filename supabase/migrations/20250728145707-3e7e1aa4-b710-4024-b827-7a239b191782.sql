-- Fix live_classes table columns
...
-- Create unique constraint on youtube_broadcast_id if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'live_classes_youtube_broadcast_id_key') THEN
    ALTER TABLE public.live_classes ADD CONSTRAINT live_classes_youtube_broadcast_id_key UNIQUE (youtube_broadcast_id);
  END IF;
END $$;
